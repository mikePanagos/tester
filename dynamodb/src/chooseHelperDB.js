...
const TABLE_NAME = "spin-the-wheel";
...
// get the table
var picksomething = dynasty.table(TABLE_NAME);
...
{
  return picksomething.remove({hash: "12345", range: "TEST"});
};


// return a category or null
// a promise is returned
choose.getCategory = function (userId, category)
{
  console.log("Get Category: "+category);
  return picksomething.find({ hash: userId, range: category });
};
...
{
  return picksomething.findAll(userId);
};
...
{
  return picksomething.remove({hash: userId, range: category});
};
